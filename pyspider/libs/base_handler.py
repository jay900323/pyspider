...
def catch_invalied_cookies(func):
    func._catch_invalied_cookies = True
    return func
...
#x相当于调用every(minutes, second)(func)(self)
...
class BaseHandlerMeta(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        #方法执行完后会在方法中设置是否周期调度和时间
        newcls = type.__new__(cls, name, bases, attrs)
        #cron_job表示是否为周期调度任务
        newcls._cron_jobs = cron_jobs
        #min_tick为多长时间发送一次调度任务
...
class BaseHandler(object):
    ...
    def _run_task(self, task, response):
        ...
        if getattr(function, '_catch_invalied_cookies', False):
            if self.is_cookie_invalied(response):
                retry, cookie = self.on_cookie_invalied(response, task)
                if retry and cookie:
                    fetch = task.get('fetch', {})
                    if 'Cookie' in fetch['headers']:
                        fetch['headers']['Cookie'] = cookie
                    if 'cookies' in fetch:
                        fetch['cookies'] = cookie
                    raise 'cookies is invalied!'
    ...
    def _crawl(self, url, **kwargs):
        ...
        if 'taskid' in kwargs:
            ...
            #modify by jinkai
            task['taskid'] = self.get_taskid(task, data)
    ...
    #判断当前响应的cookie是否无效   False表示有效  True表示无效
    #当cookie无效时会修改任务的cookie参数进行重新尝试
    def is_cookie_invalied(self, response): 
        return False
    
    #cookie失效后会调用此函数  由脚本实现
    #返回值： 是否进行重新尝试     cookie为重新登录成功后的cookie
    def on_cookie_invalied(self, response, task):
        retry = False
        cookie = {}
        return (retry, cookie)
