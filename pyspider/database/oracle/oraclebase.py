...
class SplitTableMixin(object):
    ...
    def _list_project(self):
        ...
        if self.__tablename__:
            ...
            prefix = ''
        for project, in self._execute('SELECT table_name FROM all_tables WHERE nvl(tablespace_name, \'no tablespace\') NOT IN (\'SYSTEM\', \'SYSAUX\') AND OWNER = \'%s\' AND IOT_NAME IS NULL AND DURATION IS NULL'% self.username):
