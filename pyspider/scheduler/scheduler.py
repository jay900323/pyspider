...
class Project(object):
    ...
    def update(self, project_info):
        ...
        update = self.active or self.db_status == 'CHECKING'
        if (self.md5sum != md5sum or self.waiting_get_info) and update:
...
class Scheduler(object):
    ...
    def _check_task_done(self):
        ...
        try:
            while True:
                ...
                #检查getinfo任务回调的结果 更新schedule的Project相关参数
                if task.get('taskid') == '_on_get_info' and 'project' in task and 'track' in task:
                    ...
                    continue
                self.on_task_status(task)#更新最后一次的抓取时间
                cnt += 1
        except Queue.Empty:
            pass
        return cnt
    ...
    def _check_cronjob(self):
        ...
        now = time.time()
        #初次启动时候now和self._last_tick的时间不相同 所以刚运行的时候不会启动
    ...
    def _check_select(self):
        ...
        for project in itervalues(self.projects):
            ...
            while cnt < limit and project_cnt < limit / 20:
    ...
    def run_once(self):
        ...
        #尝试更新的各种设置，例如，我们想在爬虫工作的时候调整爬取速度
        #min_tick', 'retry_delay', 'crawl_config
        self._update_projects()
        
        #分析已完成的任务并将其保存到数据库，它从status_queue中获取任务
        self._check_task_done()
        
        #如果内容处理程序要求分析更多的页面，把这些页面放在队列newtask_queue中，该函数会从该队列中获得新的任务
        #要执行的新任务会放到project对应的taskqueue里面
        #处理的都是添加在newtask_queue里的任务 不符合条件任务的丢弃
        self._check_request()
        ...
        while self._check_cronjob():
            pass
        #把新的网页加入到抓取程序的队列中
        #project的taskqueue只存储taskid等简单信息 任务信息存储在数据库中
        self._check_select()
        
        #删除已被用户标记的任务和项目。
        #如果任务被标记为delete和stop则删除
        self._check_delete()
        
        #记录一个文件中已完成任务的数量。对于防止程序异常所导致的数据丢失，这是有必要的
        self._try_dump_cnt()
