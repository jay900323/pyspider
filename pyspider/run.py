...
@click.option('--beanstalk', envvar='BEANSTALK_HOST',
              ...
              'please use --message-queue instead.')
@click.option('--phantomjs-proxy', envvar='PHANTOMJS_PROXY', help="phantomjs proxy ip:port")#phantomJs
...
@click.option('--capture_phase', default=True, help='auto restart phantomjs if crashed')
@click.pass_context
def result_worker(ctx, result_cls, capture_phase):
    """
    Run result worker.
    """
    g = ctx.obj
    ...
    result_worker = ResultWorker(resultdb=g.resultdb, inqueue=g.processor2result, capture_phase = capture_phase)
...
def phantomjs(ctx, phantomjs_path, port, auto_restart, args):
    ...
    phantomjs_fullpath = os.path.join(os.getcwd(), phantomjs_path)

    cmd = [phantomjs_fullpath,
