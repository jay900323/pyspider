...
def debug(project):
    ...
    info = projectdb.get(project, fields=['name', 'script', 'comments'])
    if info:
        script = info['script']
    else:
    ...
    if info:
        desc = info['comments'] or ''
    else:      
        desc = request.values.get('project-desc') or ''
        
    default_task['project'] = project
    return render_template("debug.html", task=task, script=script, project_name=project, project_desc=desc)
